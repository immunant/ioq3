...
unsafe extern "C" fn R_JPGErrorExit(mut cinfo: j_common_ptr) {
    ...
    longjmp((*jerr).setjmp_buffer.as_mut_ptr(), 1 as i32);
}

unsafe extern "C" fn R_JPGOutputMessage(mut cinfo: j_common_ptr) {
    let mut buffer: [libc::c_char; 200] = [0; 200];
    /* Create the message */
    Some(
        (*(*cinfo).err)
...
pub unsafe extern "C" fn R_LoadJPG(
    ...
    len = crate::src::renderergl1::tr_main::ri
        ...
        as i32;
    if fbuffer.b.is_null() || len < 0 as i32 {
        return;
    }
    ...
    if cinfo.output_width == 0
        ...
        || pixelcount
            ...
            != cinfo.output_height
        || pixelcount > 0x1fffffff as i32 as u32
        || cinfo.output_components != 3 as i32
    {
        ...
        crate::src::renderergl1::tr_main::ri
            ...
            cinfo.output_width,
            cinfo.output_height,
            pixelcount.wrapping_mul(4 as i32 as u32),
            cinfo.output_components,
        );
    }
    memcount = pixelcount.wrapping_mul(4 as i32 as u32);
    row_stride = cinfo
        .output_width
    ...
    *height = cinfo.output_height as i32;
    ...
    loop {
        ...
        *buf.offset(dindex as isize) = 255 as i32 as byte;
...
unsafe extern "C" fn init_destination(mut cinfo: j_compress_ptr) {
    ...
    (*dest).pub_0.free_in_buffer = (*dest).size as size_t;
}
/*
...
unsafe extern "C" fn term_destination(mut _cinfo: j_compress_ptr) {}
/*
...
pub unsafe extern "C" fn RE_SaveJPGToBuffer(
    ...
    cinfo.image_width = image_width as JDIMENSION; /* image width and height, in pixels */
    ...
    if quality >= 85 as i32 {
        (*cinfo.comp_info.offset(0 as i32 as isize)).h_samp_factor = 1 as i32;
    ...
    while cinfo.next_scanline < cinfo.image_height {
         ...
         * more than one scanline at a time if that's more convenient.
         */
        row_pointer[0 as i32 as usize] = &mut *image_buffer.offset(
            cinfo
                .image_height
