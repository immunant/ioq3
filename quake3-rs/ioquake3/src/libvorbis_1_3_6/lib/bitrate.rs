...
pub unsafe extern "C" fn vorbis_bitrate_addblock(mut vb: *mut vorbis_block) -> i32 {
    ...
    let mut this_bits: isize =
        oggpack_bytes((*vbi).packetblob[choice as usize] as *mut oggpack_buffer)
            * 8 as i32 as isize;
    ...
    if (*bm).avg_bitsper > 0 as i32 as isize {
        ...
        if (*bm).avg_reservoir + (this_bits - avg_target_bits) > desired_fill {
            ...
            {
                choice -= 1;
                this_bits = oggpack_bytes((*vbi).packetblob[choice as usize] as *mut oggpack_buffer)
                    * 8 as i32 as isize
            }
    ...
    if choice < 0 as i32 {
        ...
        choice = 0 as i32;
        (*bm).choice = choice;
        if oggpack_bytes((*vbi).packetblob[choice as usize] as *mut oggpack_buffer) > maxsize {
            oggpack_writetrunc(
        ...
        minsize -= oggpack_bytes((*vbi).packetblob[choice as usize] as *mut oggpack_buffer);
        loop {
            let fresh0 = minsize;
            minsize = minsize - 1;
...
pub unsafe extern "C" fn vorbis_bitrate_flushpacket(
    ...
    if !op.is_null() {
        ...
        (*op).packet =
            oggpack_get_buffer((*vbi).packetblob[choice as usize] as *mut oggpack_buffer);
        (*op).bytes = oggpack_bytes((*vbi).packetblob[choice as usize] as *mut oggpack_buffer);
