...
unsafe extern "C" fn floor1_free_info(mut i: *mut libc::c_void) {
    ...
    if !info.is_null() {
        crate::stdlib::memset(
            ...
            0 as i32,
            ::std::mem::size_of::<vorbis_info_floor1>() as usize,
        );
...
unsafe extern "C" fn floor1_free_look(mut i: *mut libc::c_void) {
    ...
    if !look.is_null() {
        ...
        crate::stdlib::memset(
            ...
            0 as i32,
            ::std::mem::size_of::<vorbis_look_floor1>() as usize,
        );
...
unsafe extern "C" fn floor1_pack(mut i: *mut libc::c_void, mut opb: *mut oggpack_buffer) {
    ...
    while j < maxclass + 1 as i32 {
        oggpack_write(
            opb as *mut oggpack_buffer,
            ((*info).class_dim[j as usize] - 1 as i32) as usize,
            3 as i32,
        ); /* 0 to 3 */
        oggpack_write(
            opb as *mut oggpack_buffer,
        ...
        while k < (1 as i32) << (*info).class_subs[j as usize] {
            oggpack_write(
                opb as *mut oggpack_buffer,
                ((*info).class_subbook[j as usize][k as usize] + 1 as i32) as usize,
                8 as i32,
            );
            k += 1
        }
    ...
    while j < (*info).partitions {
        ...
        while k < count {
            oggpack_write(
                opb as *mut oggpack_buffer,
                (*info).postlist[(k + 2 as i32) as usize] as usize,
                rangebits,
            );
            k += 1
        }
...
unsafe extern "C" fn accumulate_fit(
    ...
    crate::stdlib::memset(
        ...
        0 as i32,
        ::std::mem::size_of::<lsfit_acc>() as usize,
    );
    (*a).x0 = x0;
    (*a).x1 = x1;
    if x1 >= n {
...
pub unsafe extern "C" fn floor1_encode(
    ...
    if !post.is_null() {
        ...
        oggpack_write(
            opb as *mut oggpack_buffer,
            out[0 as i32 as usize] as usize,
        ...
        oggpack_write(
            opb as *mut oggpack_buffer,
            out[1 as i32 as usize] as usize,
            crate::src::libvorbis_1_3_6::lib::sharedbook::ov_ilog(
                ((*look).quant_q - 1 as i32) as ogg_uint32_t,
            ),
        );
...
unsafe extern "C" fn floor1_inverse2(
    ...
    crate::stdlib::memset(
        ...
        0 as i32,
        (::std::mem::size_of::<f32>() as usize).wrapping_mul(n as usize),
    );
    return 0 as i32;
}
/* export hooks */
