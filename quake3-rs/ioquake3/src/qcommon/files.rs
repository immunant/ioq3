...
static mut missionpak_checksums: [u32; 4] = [
    2430342401 as u32,
    511014160 as u32,
    2662638993 as u32,
    1438664554 as u32,
...
static mut fs_packFiles: i32 = 0 as i32;
...
unsafe extern "C" fn FS_FileForHandle(mut f: fileHandle_t) -> *mut FILE {
    ...
    if fsh[f as usize].zipFile as u32 == qtrue as i32 as u32 {
        Com_Error(
            ERR_DROP as i32,
...
pub unsafe extern "C" fn FS_fplength(mut h: *mut FILE) -> isize {
    let mut pos: isize = 0;
    let mut end: isize = 0;
    pos = ftell(h);
    fseek(h, 0 as i32 as isize, 2 as i32);
    end = ftell(h);
    fseek(h, pos, 0 as i32);
    return end;
}
/*
================
...
pub unsafe extern "C" fn FS_BuildOSPath(
    ...
    toggle ^= 1 as i32;
    if game.is_null() || *game.offset(0 as i32 as isize) == 0 {
...
pub unsafe extern "C" fn FS_CreatePath(mut OSPath: *mut libc::c_char) -> qboolean {
    ...
    while !ofs.is_null() && *ofs as i32 != 0 {
        if *ofs as i32 == '/' as i32 {
            ...
            *ofs = 0 as i32 as libc::c_char;
            if Sys_Mkdir(path.as_mut_ptr()) as u64 == 0 {
                Com_Error(
                    ERR_FATAL as i32,
...
pub unsafe extern "C" fn FS_SV_FileExists(mut file: *const libc::c_char) -> qboolean {
    ...
    *testpath.offset(crate::stdlib::strlen(testpath).wrapping_sub(1 as i32 as usize) as isize) =
...
pub unsafe extern "C" fn FS_SV_FOpenFileWrite(mut filename: *const libc::c_char) -> fileHandle_t {
    ...
    *ospath.offset(crate::stdlib::strlen(ospath).wrapping_sub(1 as i32 as usize) as isize) =
        '\u{0}' as i32 as libc::c_char;
    f = FS_HandleForFile();
...
pub unsafe extern "C" fn FS_SV_FOpenFileRead(
    ...
    *ospath.offset(crate::stdlib::strlen(ospath).wrapping_sub(1 as i32 as usize) as isize) =
    ...
    if fsh[f as usize].handleFiles.file.o.is_null() {
        ...
        // Check fs_steampath
        if fsh[f as usize].handleFiles.file.o.is_null()
            && *(*fs_steampath).string.offset(0 as i32 as isize) as i32 != 0
        {
            ospath = FS_BuildOSPath(
                (*fs_steampath).string,
                filename,
        ...
        // Check fs_gogpath
        if fsh[f as usize].handleFiles.file.o.is_null()
            && *(*fs_gogpath).string.offset(0 as i32 as isize) as i32 != 0
        {
            ospath = FS_BuildOSPath(
                (*fs_gogpath).string,
                filename,
...
pub unsafe extern "C" fn FS_SV_Rename(
    ...
    *from_ospath
        .offset(crate::stdlib::strlen(from_ospath).wrapping_sub(1 as i32 as usize) as isize) =
        '\u{0}' as i32 as libc::c_char;
    *to_ospath.offset(crate::stdlib::strlen(to_ospath).wrapping_sub(1 as i32 as usize) as isize) =
        '\u{0}' as i32 as libc::c_char;
    if (*fs_debug).integer != 0 {
        Com_Printf(
            b"FS_SV_Rename: %s --> %s\n\x00" as *const u8 as *const libc::c_char,
...
pub unsafe extern "C" fn FS_FCreateOpenPipeFile(mut filename: *const libc::c_char) -> fileHandle_t {
    ...
    fifo = Sys_Mkfifo(ospath) as *mut _IO_FILE;
    if !fifo.is_null() {
...
pub unsafe extern "C" fn FS_FOpenFileReadDir(
    ...
    if com_fullyInitialized as u32 != 0
        ...
        if !(*(*(*search).pack).hashTable.offset(hash as isize)).is_null() {
            ...
            loop {
                ...
                if FS_FilenameCompare((*pakFile).name, filename) as u64 == 0 {
                    ...
                    if (*pak).referenced & 0x1 as i32 == 0 {
                        if FS_IsExt(
                            filename,
...
pub unsafe extern "C" fn FS_Read(
    ...
    fs_readCount += len;
    if fsh[f as usize].zipFile as u32 == qfalse as i32 as u32 {
        remaining = len;
        tries = 0 as i32;
        while remaining != 0 {
            block = remaining;
...
pub unsafe extern "C" fn FS_Seek(mut f: fileHandle_t, mut offset: isize, mut origin: i32) -> i32 {
    ...
    if fsh[f as usize].zipFile as u32 == qtrue as i32 as u32 {
        ...
        if offset < 0 as i32 as isize {
            ...
            origin = FS_SEEK_SET as i32
        } else if origin == FS_SEEK_END as i32 {
        ...
        //fallthrough
        while remainder > 65536 as i32 {
            FS_Read(buffer.as_mut_ptr() as *mut libc::c_void, 65536 as i32, f);
            remainder -= 65536 as i32
        }
...
pub unsafe extern "C" fn FS_ReadFileDir(
    ...
    if qpath.is_null() || *qpath.offset(0 as i32 as isize) == 0 {
        Com_Error(
            ERR_FATAL as i32,
...
unsafe extern "C" fn FS_LoadZipFile(
    ...
    while (i as usize) < gi.number_entry {
        ...
        len = (len as usize).wrapping_add(
            crate::stdlib::strlen(filename_inzip.as_mut_ptr()).wrapping_add(1 as i32 as usize),
        ) as i32;
        unzGoToNextFile(uf);
        i += 1
    }
    ...
    *fs_headerLongs.offset(fresh2 as isize) = fs_checksumFeed;
    ...
    pack = Z_Malloc((::std::mem::size_of::<pack_t>() as usize).wrapping_add(
        (i as usize).wrapping_mul(::std::mem::size_of::<*mut fileInPack_t>() as usize),
    ) as i32) as *mut pack_t;
    (*pack).hashSize = i;
    ...
    (*pack).pure_checksum = Com_BlockChecksum(
        ...
        (::std::mem::size_of::<i32>() as usize).wrapping_mul(fs_numHeaderLongs as usize) as i32,
    ) as i32;
...
unsafe extern "C" fn FS_ReturnPath(
    ...
    newdep = 0 as i32;
    *zpath.offset(0 as i32 as isize) = 0 as i32 as libc::c_char;
    len = 0 as i32;
    at = 0 as i32;
    while *zname.offset(at as isize) as i32 != 0 as i32 {
    ...
    *zpath.offset(len as isize) = 0 as i32 as libc::c_char;
    *depth = newdep;
    return len;
}
/*
...
unsafe extern "C" fn FS_AddFileToList(
    ...
    i = 0 as i32;
    while i < nfiles {
        if Q_stricmp(name, *list.offset(i as isize)) == 0 {
            return nfiles;
            // already in list
        }
        i += 1
...
pub unsafe extern "C" fn FS_ListFilteredFiles(
    ...
    if *path.offset((pathLength - 1 as i32) as isize) as i32 == '\\' as i32
        || *path.offset((pathLength - 1 as i32) as isize) as i32 == '/' as i32
    {
        pathLength -= 1
    }
...
pub unsafe extern "C" fn FS_GetFileList(
    ...
    *listbuf = 0 as i32 as libc::c_char;
    ...
    i = 0 as i32;
    while i < nFiles {
        nLen = crate::stdlib::strlen(*pFiles.offset(i as isize)).wrapping_add(1 as i32 as usize)
            as i32;
...
unsafe extern "C" fn Sys_CountFileList(mut list: *mut *mut libc::c_char) -> u32 {
...
unsafe extern "C" fn Sys_ConcatenateFileLists(
    ...
    *dst = std::ptr::null_mut();
    ...
    if !list0.is_null() {
        Z_Free(list0 as *mut libc::c_void);
    }
    if !list1.is_null() {
        Z_Free(list1 as *mut libc::c_void);
    }
    return cat;
}
/*
...
pub unsafe extern "C" fn FS_GetModList(mut listbuf: *mut libc::c_char, mut bufsize: i32) -> i32 {
    let mut nMods: i32 = 0;
    let mut i: i32 = 0;
    let mut j: i32 = 0;
    let mut k: i32 = 0;
...
pub unsafe extern "C" fn FS_SortFileList(mut filelist: *mut *mut libc::c_char, mut numfiles: i32) {
    let mut i: i32 = 0;
    let mut j: i32 = 0;
    let mut k: i32 = 0;
    ...
    *fresh13 = std::ptr::null_mut();
    ...
    while i < numfiles {
        ...
        while j < numsortedfiles {
            if FS_PathCmp(*filelist.offset(i as isize), *sortedlist.offset(j as isize)) < 0 as i32 {
                break;
            }
            j += 1
        }
        ...
        while k > j {
            ...
            *fresh14 = *sortedlist.offset((k - 1 as i32) as isize);
            k -= 1
        }
    ...
    crate::stdlib::memcpy(
        ...
        (numfiles as usize).wrapping_mul(::std::mem::size_of::<*mut libc::c_char>() as usize),
    );
...
pub unsafe extern "C" fn FS_AddGameDirectory(
    ...
    while !sp.is_null() {
        if !(*sp).dir.is_null()
            && Q_stricmp((*(*sp).dir).path.as_mut_ptr(), path) == 0
            && Q_stricmp((*(*sp).dir).gamedir.as_mut_ptr(), dir) == 0
        {
            return;
    ...
    curpath[crate::stdlib::strlen(curpath.as_mut_ptr()).wrapping_sub(1 as i32 as usize) as usize] =
        '\u{0}' as i32 as libc::c_char;
    // Get .pk3 files
    ...
    while pakfilesi < numfiles || pakdirsi < numdirs {
        ...
        if pakwhich != 0 {
                ...
                == 0
            {
                ...
                (*search).dir = Z_Malloc(::std::mem::size_of::<directory_t>() as usize as i32)
    ...
    // add the directory to the search path
    //
    search = Z_Malloc(::std::mem::size_of::<searchpath_t>() as usize as i32) as *mut searchpath_t;
    (*search).dir =
...
pub unsafe extern "C" fn FS_ComparePaks(
    ...
    *neededpaks = 0 as i32 as libc::c_char;
    i = 0 as i32;
...
pub unsafe extern "C" fn FS_Shutdown(mut _closemfp: qboolean) {
    ...
    while !p.is_null() {
        ...
        if !(*p).dir.is_null() {
            Z_Free((*p).dir as *mut libc::c_void);
        }
...
unsafe extern "C" fn FS_Startup(mut gameName: *const libc::c_char) {
    ...
    if homePath.is_null() || *homePath.offset(0 as i32 as isize) == 0 {
    ...
    if *(*fs_gogpath).string.offset(0 as i32 as isize) != 0 {
        FS_AddGameDirectory((*fs_gogpath).string, gameName);
    }
    fs_steampath = Cvar_Get(
        b"fs_steampath\x00" as *const u8 as *const libc::c_char,
        Sys_SteamPath(),
        0x10 as i32 | 0x2000 as i32,
    ) as *mut cvar_s;
    if *(*fs_steampath).string.offset(0 as i32 as isize) != 0 {
        FS_AddGameDirectory((*fs_steampath).string, gameName);
    }
    if *(*fs_basepath).string.offset(0 as i32 as isize) != 0 {
    ...
    if *(*fs_basegame).string.offset(0 as i32 as isize) as i32 != 0
    ...
    if *(*fs_gamedirvar).string.offset(0 as i32 as isize) as i32 != 0
    ...
    if (*com_standalone).integer == 0 {
        ...
        if *(*fs_gamedirvar).string.offset(0 as i32 as isize) != 0 {
...
unsafe extern "C" fn FS_CheckPak0() {
    ...
    while !path.is_null() {
        if !(*path).pack.is_null() {
            ...
            {
                if (*curpack).checksum as u32 == 2985612116 as u32 {
                    founddemo = qtrue
                }
            } else if Q_stricmpn(
                (*curpack).pakGamename.as_mut_ptr(),
                ...
                && *pakBasename.offset(3 as i32 as isize) as i32 >= '0' as i32
                && *pakBasename.offset(3 as i32 as isize) as i32 <= '0' as i32 + 9 as i32 - 1 as i32
            {
                if (*curpack).checksum as u32
                    != pak_checksums
                        [(*pakBasename.offset(3 as i32 as isize) as i32 - '0' as i32) as usize]
                {
                    if *pakBasename.offset(3 as i32 as isize) as i32 == '0' as i32 {
                        ...
                        Com_Printf(b"\n\n**************************************************\nWARNING: baseq3/pak%d.pk3 is present but its checksum (%u)\nis not correct. Please re-install the point release\n**************************************************\n\n\n\x00"
                                       as *const u8 as *const libc::c_char,
                                   *pakBasename.offset(3 as i32 as
                                                           isize) as
                                       i32 - '0' as i32,
                                   (*curpack).checksum);
                    }
                ...
                && *pakBasename.offset(3 as i32 as isize) as i32 >= '0' as i32
                && *pakBasename.offset(3 as i32 as isize) as i32 <= '0' as i32 + 4 as i32 - 1 as i32
            {
                if (*curpack).checksum as u32
                    != missionpak_checksums
                        [(*pakBasename.offset(3 as i32 as isize) as i32 - '0' as i32) as usize]
                {
                    Com_Printf(b"\n\n**************************************************\nWARNING: missionpack/pak%d.pk3 is present but its checksum (%u)\nis not correct. Please re-install Team Arena\n**************************************************\n\n\n\x00"
                                   as *const u8 as *const libc::c_char,
                               *pakBasename.offset(3 as i32 as isize)
                                   as i32 - '0' as i32,
                               (*curpack).checksum);
                }
                foundTA |= ((1 as i32)
    ...
    if (*com_standalone).integer == 0
        && foundTA != 0
        && foundTA & 0xf as i32 as u32 != 0xf as i32 as u32
    {
    ...
    while nFlags != 0 {
        ...
        while !search.is_null() {
            ...
            if !(*search).pack.is_null() && (*(*search).pack).referenced & nFlags != 0 {
                ...
                if nFlags & (0x4 as i32 | 0x2 as i32) != 0 {
                    break;
                }
...
pub unsafe extern "C" fn FS_PureServerSetLoadedPaks(
    ...
    i = 0 as i32;
    while i < c {
        fs_serverPaks[i as usize] = atoi(Cmd_Argv(i));
        i += 1
    }
    if fs_numServerPaks != 0 {
...
pub unsafe extern "C" fn FS_Restart(mut checksumFeed: i32) {
    ...
    ) <= 0 as i32 as isize
    {
        ...
        if lastValidBase[0 as i32 as usize] != 0 {
            FS_PureServerSetLoadedPaks(
                b"\x00" as *const u8 as *const libc::c_char,
                b"\x00" as *const u8 as *const libc::c_char,
            );
            ...
            lastValidBase[0 as i32 as usize] = '\u{0}' as i32 as libc::c_char;
            lastValidComBaseGame[0 as i32 as usize] = '\u{0}' as i32 as libc::c_char;
            lastValidFsBaseGame[0 as i32 as usize] = '\u{0}' as i32 as libc::c_char;
            lastValidGame[0 as i32 as usize] = '\u{0}' as i32 as libc::c_char;
    ...
    lastGameDir = if lastValidGame[0 as i32 as usize] as i32 != 0 {
        lastValidGame.as_mut_ptr()
    } else {
        lastValidComBaseGame.as_mut_ptr()
    };
    if Q_stricmp(FS_GetCurrentGameDir(), lastGameDir) != 0 {
...
pub unsafe extern "C" fn FS_FOpenFileByMode(
    ...
    match mode as u32 {
        0 => {
            r = FS_FOpenFileRead(qpath, f, qtrue) as i32;
...
pub unsafe extern "C" fn FS_FTell(mut f: fileHandle_t) -> i32 {
    let mut pos: i32 = 0;
    if fsh[f as usize].zipFile as u32 == qtrue as i32 as u32 {
        pos = unztell(fsh[f as usize].handleFiles.file.z) as i32
    } else {
        pos = ftell(fsh[f as usize].handleFiles.file.o) as i32
    }
    return pos;
}
#[no_mangle]
