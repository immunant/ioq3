...
pub unsafe extern "C" fn BotGetAirGoal(
    ...
    if bsptrace.fraction > 0 as i32 as f32 {
        ...
        if areanum != 0 {
            ...
            (*goal).mins[0 as i32 as usize] = -(15 as i32) as vec_t;
            (*goal).mins[1 as i32 as usize] = -(15 as i32) as vec_t;
            (*goal).mins[2 as i32 as usize] = -(1 as i32) as vec_t;
            (*goal).maxs[0 as i32 as usize] = 15 as i32 as vec_t;
            (*goal).maxs[1 as i32 as usize] = 15 as i32 as vec_t;
            (*goal).maxs[2 as i32 as usize] = 1 as i32 as vec_t;
...
pub unsafe extern "C" fn BotGoForAir(
    ...
    if (*bs).lastair_time < floattime - 6 as i32 as f32 {
        //
        //DEBUG
...
pub unsafe extern "C" fn BotReachedGoal(
    ...
    if (*goal).flags & 1 as i32 != 0 {
        ...
        if trap_BotItemGoalInVisButNotVisible(
            (*bs).entitynum,
            (*bs).eye.as_mut_ptr(),
...
pub unsafe extern "C" fn BotGetItemLongTermGoal(
    ...
    if trap_BotGetTopGoal((*bs).gs, goal as *mut libc::c_void) == 0 {
    ...
    if (*bs).ltg_time < floattime {
        ...
        ) != 0
        {
            ...
            */
            (*bs).ltg_time = floattime + 20 as i32 as f32
        } else {
...
pub unsafe extern "C" fn BotGetLongTermGoal(
    ...
    let mut botinfo: aas_entityinfo_t = aas_entityinfo_t {
        valid: 0,
        type_0: 0,
        flags: 0,
        ltime: 0.,
    ...
    if (*bs).ltgtype == 1 as i32 && retreat == 0 {
        ...
        if (*bs).teammatevisible_time < floattime - 10 as i32 as f32 {
            (*bs).ltgtype = 0 as i32
        }
    ...
    if (*bs).ltgtype == 2 as i32 && retreat == 0 {
        ...
        ) != 0.
        {
                ...
                < (*bs).formation_dist * (*bs).formation_dist
            {
                ...
                BotEntityInfo(
                    (*bs).entitynum,
                    &mut botinfo as *mut _ as *mut aas_entityinfo_s,
                );
                ...
                if (*bs).arrive_time < floattime - 2 as i32 as f32 {
                    //if not arrived yet
                    if (*bs).arrive_time == 0. {
                ...
                if BotGoForAir(bs, (*bs).tfl, &mut (*bs).teamgoal, 400 as i32 as f32) != 0 {
                    ...
                    (*bs).nbg_time = floattime + 8 as i32 as f32;
                    AIEnter_Seek_NBG(
                        bs,
...
pub unsafe extern "C" fn BotLongTermGoal(
    ...
    if (*bs).lead_time > 0 as i32 as f32 && retreat == 0 {
        ...
        if (*bs).leadvisible_time < floattime - 1 as i32 as f32 {
            (*bs).leadbackup_time = floattime + 2 as i32 as f32
        }
...
pub unsafe extern "C" fn BotClearPath(
    ...
    if (*moveresult).flags & 256 as i32 != 0 {
        (*bs).blockedbyavoidspot_time = floattime + 5 as i32 as f32
    }
...
pub unsafe extern "C" fn AINode_Seek_ActivateEntity(
    ...
    if targetvisible != 0 {
        ...
        crate::src::game::ai_dmq3::BotAIBlocked(
            bs as *mut bot_state_s,
            &mut moveresult as *mut _ as *mut bot_moveresult_s,
            qtrue as i32,
        );
    }
    //
...
pub unsafe extern "C" fn AINode_Seek_NBG(
    ...
    if (*bs).nbg_time < floattime {
        ...
        (*bs).check_time = (floattime as f64 + 0.05f64) as f32;
        //go back to seek ltg
        AIEnter_Seek_LTG(
            bs,
    ...
    if crate::src::game::ai_dmq3::BotAIPredictObstacles(
        bs as *mut bot_state_s,
        &mut goal as *mut _ as *mut bot_goal_s,
    ) != 0
    {
        return qfalse as i32;
    }
...
pub unsafe extern "C" fn AINode_Seek_LTG(
    ...
    (*bs).enemy = -(1 as i32);
    //
    if (*bs).killedenemy_time > floattime - 2 as i32 as f32 {
        if ((libc::rand() & 0x7fff as i32) as f32 / 0x7fff as i32 as f32)
...
pub unsafe extern "C" fn AINode_Battle_Chase(
    ...
    //check for nearby goals periodicly
    if (*bs).check_time < floattime {
        (*bs).check_time = floattime + 1 as i32 as f32;
...
pub unsafe extern "C" fn AINode_Battle_Retreat(
    ...
    if (*bs).enemyvisible_time < floattime - 4 as i32 as f32 {
        AIEnter_Seek_LTG(
            bs,
